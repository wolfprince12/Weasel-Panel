...
`./build.sh` 包含 **8 道关卡**，缺一不放过（外加 GitHub CI 第 9 道）：
...
7. **XAML 默认 TwoWay 绑定 × VM 只读属性体检**（`tools/check_binding_readonly.py`，拦 `<Run Text="{Binding X}"/>` 撞 `public T X => ...;` / `{ get; }` / `public static readonly T X = ...` 这类启动抛 `InvalidOperationException` CheckReadOnly 或 silently 不显示的雷区）
8. **出包验收**（`tools/verify_lang_packs.py` 拿每包哨兵原文真搜 exe）
...
> v0.2.4 / v0.2.5 / v0.2.5(第五类) 真机连续栽了 **5 类**「编译期不报真机才炸」的 XAML 错才把这 5 道 lint 配齐：Hyperlink 容器类型、`{StaticResource}` 误用 VM const、VM string 给 XAML Uri、Binding 路径错、**默认 TwoWay target 撞 VM 只读属性**。macOS 交叉编译**完全看不见**纯运行时 XAML 错，这 5 道 lint + CI 是唯一兜底。
